...
def api_check_goal():
    ...
    months_needed = (goal_amount / current_savings) if current_savings>0 else float("inf")
    suggestions = []
    if not feasible:
        deficit = needed_savings - current_savings
        top_cats = list(analysis.get("category_totals",{}).items())[:3]
        for cat, amt in top_cats:
            if amt>0: suggestions.append(f"Reduce {cat} by 10% (≈ ₹{round(amt*0.1,2)})")
        suggestions.append(f"Save ₹{round(deficit,2)} more per month to meet goal.")
    else:
    ...
    return jsonify({
        ...
        "months_needed_at_current_rate": round(months_needed,1) if months_needed!=float("inf") else None,
